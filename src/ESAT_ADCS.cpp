...
{
  addTelemetryPacket(ESAT_ADCSACKTelemetryPacket);
...
void ESAT_ADCSClass::handleTelecommand(ESAT_CCSDSPacket &packet)
{
  ...
  if (primaryHeader.applicationProcessIdentifier != getApplicationProcessIdentifier())
  {
    return;
  }
       ...
       handler = handler->nextTelecommandHandler)
  {
    ...
    if (handled)
    {
      ESAT_ADCSACKTelemetryPacket.saveSecondaryHeader(packet);
      bool ACKAvailable = true;
      ESAT_ADCSACKTelemetryPacket.makeAvailable(ACKAvailable);
      return;
    }
  }
}
